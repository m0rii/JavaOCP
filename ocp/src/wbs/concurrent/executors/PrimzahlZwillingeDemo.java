...
public class PrimzahlZwillingeDemo {
        ...
        public static void main(String[] args) throws InterruptedException,
                ...
                PrimzahlZwillingeTask primeTask;

                Collection<PrimzahlZwillingeTask> tasks = new HashSet<>();

                for (int i = 1; i <= numOfIntervals; i++) {
                        primeTask = new PrimzahlZwillingeTask(startWert, startWert.add(
                                interval).subtract(two));
                        tasks.add(primeTask);
                        startWert = startWert.add(interval);
                }

                List<Future<PrimzahlZwillingeTaskResult>> result = executor.invokeAll(
                        tasks);
                executor.shutdown();
                NavigableSet<BigInteger> twinPrimes = new TreeSet<>();
                for (Future<PrimzahlZwillingeTaskResult> future : result) {
